...
/**
 * This class is the element that ties {@link RecyclerView.Adapter} together with {@linkAdapterDelegate}.
 * <p>
 * So you have to add / register your {@link AdapterDelegate}s to this manager by calling {@link #addDelegate(AdapterDelegate)}
 * </p>
 * Next you have to add this AdapterDelegateManager to the {@link RecyclerView.Adapter} by calling corresponding methods:
 * <ul>
 * <li> {@link #getItemViewType(int, T)}: Must be called from {@link RecyclerView.Adapter#getItemViewType(int)}</li>
 * <li> {@link #onCreateViewHolder(ViewGroup, int)}: Must be called from {@link RecyclerView.Adapter#onCreateViewHolder(ViewGroup, int)}</li>
 * <li> {@link #onBindViewHolder(VH, int, T)}: Must be called from {@link RecyclerView.Adapter#onBindViewHolder(VH, int)}</li>
 * </ul>
 * <p>
 * You can also set a fallback {@link AdapterDelegate} by using
 * {@link #setFallbackDelegate(AdapterDelegate)} that will be used if no {@link AdapterDelegate} is
 ...
 */
public class AdapterDelegateManager<T, VH extends RecyclerView.ViewHolder> {
     ...
     */
    SparseArrayCompat<AdapterDelegate<T, VH>> delegates = new SparseArrayCompat();
    private AdapterDelegate<T, VH> fallbackDelegate;
     ...
     */
    public AdapterDelegateManager<T, VH> addDelegate(@NonNull AdapterDelegate<T, VH> delegate) {
     ...
     * @see #addDelegate(int, boolean, AdapterDelegate)
     */
    public AdapterDelegateManager<T, VH> addDelegate(int viewType,
     ...
     *                               same view type.
     * @param delegate               The delegate to add
     * @throws IllegalArgumentException if <b>allowReplacingDelegate</b>  is false and an {@link AdapterDelegate}
     *                                  is already added (registered) with the same ViewType.
     * @throws IllegalArgumentException if viewType is {@link #FALLBACK_DELEGATE_VIEW_TYPE} which is reserved
     * @see #addDelegate(AdapterDelegate)
     * @see #addDelegate(int, AdapterDelegate)
     * @see #setFallbackDelegate(AdapterDelegate)
     */
    public AdapterDelegateManager<T, VH> addDelegate(int viewType, boolean allowReplacingDelegate,
                                                     @NonNull AdapterDelegate<T, VH> delegate) {
     ...
     * @return self
     */
    public AdapterDelegateManager<T, VH> removeDelegate(@NonNull AdapterDelegate<T, VH> delegate) {
     ...
     * @return self
     */
    public AdapterDelegateManager<T, VH> removeDelegate(int viewType) {
     ...
     *                                  responsible for the given data element in data set (No {@link AdapterDelegate} for the given ViewType)
    ...
    /**
     * Must be called from{@link RecyclerView.Adapter#onBindViewHolder(VH, int)}
     *
    ...
    public void onBindViewHolder(@NonNull VH viewHolder,
        ...
        delegate.onBindViewHolder(viewHolder, position, t);
    }
     ...
     *                         null if you want to remove a previously set fallback AdapterDelegate
     */
    public AdapterDelegateManager<T, VH> setFallbackDelegate(
            @Nullable AdapterDelegate<T, VH> fallbackDelegate) {
     ...
     *
     * @param viewType The view type integer we want to retrieve the associated delegate for.
     ...
     */
    @Nullable
    public AdapterDelegate<T, VH> getFallbackDelegate() {
        return fallbackDelegate;
    }
}
